...
class Audio_Dataset(torch.utils.data.Dataset):
    def __init__(self, audio_df, label_dict):
    ...
    def __getitem__(self, index):
        filename, label = self.file_list[index], self.label_list[index]
...
def prepare_dataloader(batch_size=256):
    ...
    train_dataset = Audio_Dataset(train_df, label_dict)
    dev_dataset = Audio_Dataset(dev_df, label_dict)
    eval_dataset = Audio_Dataset(eval_df, label_dict)
...
def train_step(model, data_loader, configs, device):
    ...
    optimizer = optim.SGD(model.parameters(), lr=configs["learing_rate"])

    train_loss_list, true_label_list, pre_label_list = [], [], []
    pbar = tqdm(data_loader)
    pbar.set_description(f"Traininng")
    for file_list, label_list in pbar:
        # extract features
        feature_list = []
        ...
        # forward
        train_inputs = torch.Tensor(feature_list).to(device)
...
def eval_step(model, data_loader, configs, device):
    ...
    pbar = tqdm(data_loader)
    pbar.set_description(f"evalinng")
    with torch.no_grad():
        for file_list, label_list in pbar:
            # extract features
            feature_list = []
            ...
            # forward
            inputs = torch.Tensor(feature_list).to(device)
...
def train(model, train_loader, dev_loader, configs, device, modelpath, max_epoch=200, early_stop=3):
    ...
    while epoch < max_epoch:
        ...
        # eval on dev set
        dev_loss, dev_acc = eval_step(model, dev_loader, configs, device)
        ...
        if dev_acc > best_acc :
            ...
            new_model_name = "model_{}_ep{}.pkl".format(formatted_today, best_epoch)
